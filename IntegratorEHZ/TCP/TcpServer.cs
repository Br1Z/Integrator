...
namespace IntegratorEHZ
{
    ...
    public class TcpServer : IDisposable
    {
        ...
        protected readonly ConcurrentDictionary<int, TcpSession> SessionsDeviceIdList = new ConcurrentDictionary<int, TcpSession>();
        ...
        /// <summary>
        /// Находит сессию по выданному идентификатору устройства
        /// </summary>
        /// <param name="idSession"></param>
        /// <returns>Сессию с выданным идентификатором устройства</returns>
        public TcpSession FindSession(int idSession)
        {
            return SessionsDeviceIdList.TryGetValue(idSession, out TcpSession result) ? result : null;
        }
        ...
        /// <summary>
        /// Регистрация id устройства текущей сессии
        /// </summary>
        /// <param name="session">Session to register</param>
        internal void RegisterIdDevice(TcpSession session)
        {
            // Register a new session
            SessionsDeviceIdList.TryAdd(session.DeviceIdCurrentSession, session);
        }
        ...
        /// <summary>
        /// Отменяет регистриацию id устройства текущей сессии
        /// </summary>
        /// <param name="id">Session Id</param>
        internal void UnregisterIdDevice(int id)
        {
            // Unregister session by Id
            SessionsDeviceIdList.TryRemove(id, out TcpSession temp);
        }
        #endregion
